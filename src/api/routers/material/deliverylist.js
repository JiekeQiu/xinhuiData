    ...
    if(ctx.query.type==1){
        ...
        for(let i=0;i<data.length;i++){
            ...
            if(res.length>0){
                ...
                // nums.toFixed(2)
                let sjNum = nums.toFixed(2)
            console.log("看看相减后的数据",nums)
                _id = new ObjectID(res[0]._id)
                let arr = await db.update("materialGoods", { _id }, { $set: { num: sjNum } })
        ...
        for(let i=0;i<data.length;i++){
            ...
            if(res.length>0){
               ...
               let sjNum= nums.toFixed(2)
                _id = new ObjectID(res[0]._id)
                let arr = await db.update("hardwareGoods", { _id }, { $set: { num: sjNum } })
                if (arr.modifiedCount > 0) {
                     // 直接把数据传到历史记录表里
                     let res = await db.insert('hardwaredelivery', { name: data[i].name, typeName: data[i].typeName, num: data[i].num, unit: data[i].unit,address:data[i].address, price: data[i].price, money: data[i].money, remark: data[i].remark, username, time ,operation})
                     if (res.acknowledged) {
                         ctx.body = {
                             state: 200,
                             msg: "出库成功"
